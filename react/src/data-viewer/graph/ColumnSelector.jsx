...
const COLUMN_CONFIG = {
  ...
  histogram: [{ key: "column", label: "Column", types: ["numeric"], multi: false }],
  heatmap: [
    { key: "x", label: "X Axis", types: ["categorical", "numeric", "date"], multi: false },
...
export default function ColumnSelector({
  ...
  const columnTypes = useMemo(() => inferAllColumnTypes(rows, headers), [rows, headers]);
  ...
  return (
    <div className="flex flex-wrap gap-4">
      {config.map((field) => {
        const eligible = columnTypes.filter((col) => field.types.includes(col.type));
          ...
          return (
            <div key={field.key}>
              ...
              <div className="flex flex-wrap gap-1">
                {eligible.length === 0 && (
                  <span className="text-xs text-slate-400">No {field.types.join("/")} columns</span>
                )}
                {eligible.map((col) => {
                  const checked = (selectedColumns[field.key] || []).includes(col.name);
                  return (
                    <label
                      key={col.name}
